<template>
  <v-form v-model="valid" ref="heBrandForm">
    <v-text-field v-model="brand.name" :rules="nameRules" label="请输入品牌名称" required/>
    <v-text-field v-model="brand.letter" :rules="letterRules" label="请输入品牌首字母" required/>
    <v-cascader
      url="/item/category/list"
      multiple
      required
      v-model="brand.categories"
      label="请选择商品分类"/>
    <v-layout row>
      <v-flex xs3>
        <span style="font-size: 16px; color: #444">品牌LOGO：</span>
      </v-flex>
      <v-flex>
        <v-upload
          v-model="brand.image"
          url="/upload/image"
          :multiple="false"
          :pic-width="250"
          :pic-height="90"
        />
      </v-flex>
    </v-layout>
    <v-layout class="my-4" row>
      <v-spacer/>
      <v-btn @click="submit" color="primary">提交</v-btn>
      <v-btn @click="clear">重置</v-btn>
    </v-layout>

  </v-form>
</template>

<script>
  export default {
    name: "HeBrandForm",
    props:{
      oldBrand:{
        type:Object
      }
    },
    data() {
      return {
        valid: false, // 表单校验结果标记
        brand: {
          name: '', // 品牌名称
          letter: '', // 品牌首字母
          image: '',// 品牌logo
          categories: [], // 品牌所属的商品分类数组
        },
        nameRules: [
          v => !!v || "品牌名称必填！",
          v => v.length > 1 || "品牌名称至少2位"
        ],
        letterRules: [
          v => !!v || "首字母不能为空",
          v => /^[A-Z]{1}$/.test(v) || "品牌字母只能是A~Z的大写字母"
        ]
      }
    },
    watch:{
      oldBrand:{
        deep:true,
        handler(val){
          if(val){
            // 注意不要直接复制，否则这边的修改会影响到父组件的数据，copy属性即可
            this.brand =  Object.deepCopy(val)
          }else{
            // 为空，初始化brand
            this.brand = {
              name: '',
              letter: '',
              image: '',
              categories: [],
            }
          }
        }
      }
    },
    methods: {
      submit() {
        //判断表单是否验证成功
        if (this.$refs.heBrandForm.validate()) {
          const {categories, ...params} = this.brand;
          params.cids = categories.map(c => c.id).join(",");

          this.$http.post("/item/brand", this.$qs.stringify(params)).then((resp) => {
            // 6、弹出提示
            this.$message.success("保存成功！");

            //关闭窗口
           this.$emit("close")
          }).catch((err)=>{
            this.$message.error("保存失败！");
          })
        }
      },
      clear() {
        // 重置表单
        this.$refs.heBrandForm.reset();
        // 手动重置
        this.brand.categories = [];
      }
    }

  }
</script>

<style scoped>

</style>
